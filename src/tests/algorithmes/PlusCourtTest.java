	...
	void testDijkstra() throws NumberFormatException, IOException {
		...
		for (String fichier : listeFichiers) {
			try {
				...
				assertThrows(NoPathEx.class,()-> {GraphImporter.comparer(REPERTOIRE_ENONCE+REPERTOIRE_DIJKSTRA + fichier, REPERTOIRE_REPONSE+REPERTOIRE_DIJKSTRA + fichier.replace('g', 'r'), algo);});
			}
		}
	}
	...
	void testBellman() throws NumberFormatException, IOException {
		...
		for (String fichier : listeFichiers) {
			try{
				...
				assertThrows(NoPathEx.class,()-> {GraphImporter.comparer(REPERTOIRE_ENONCE+REPERTOIRE_BELLMAN + fichier, REPERTOIRE_REPONSE+REPERTOIRE_BELLMAN + fichier.replace('g', 'r'), algo);});
			}
		}
	}
}
